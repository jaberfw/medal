// ============================================================
// Medal Gifts - Dynamic Image Auto-Detection Script
// ============================================================
...
// Probes image URLs: prefix1.jpg, prefix2.jpg ... until miss
// Stops after 3 consecutive misses to handle gaps
// ============================================================
function probeImage(url) {
  return new Promise(resolve => {
    ...
    img.src = url + '?t=' + Date.now(); // avoid cache issues
  });
}

async function detectImages(prefix, maxCheck = 60) {
  const found = [];
  let misses = 0;
  for (let i = 1; i <= maxCheck; i++) {
    let hit = false;
    for (const ext of ['jpg', 'jpeg', 'png', 'webp', 'JPG', 'PNG']) {
      const url = `./assets/images/${prefix}${i}.${ext}`;
      if (await probeImage(url)) { found.push(url.split('?')[0]); hit = true; break; }
    }
...
async function buildProducts() {
  ...
  for (const cat of CATEGORY_CONFIG) {
    const images = await detectImages(cat.prefix);
...
function renderGallery() {
  ...
  if (paginated.length === 0) {
    productGrid.innerHTML = `<p style="grid-column:1/-1;text-align:center;padding:4rem;color:#888;">No products found. Add images named like pin1.jpg, pin2.jpg into assets/images/</p>`;
...
// ============================================================
// HOME: FEATURED GRID (1 image per category = 10 cards, 3-col)
// ============================================================
async function renderFeatured() {
  ...
  const featured = [];
  for (const cat of CATEGORY_CONFIG) {
    const images = await detectImages(cat.prefix, 5);
...
async function updateCategoryImages() {
  for (const cat of CATEGORY_CONFIG) {
    ...
    if (!el) continue;
    const images = await detectImages(cat.prefix, 3);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (productGrid) {
    productGrid.innerHTML = `
      <div style="grid-column:1/-1;text-align:center;padding:5rem;color:#999;">
        ...
        <p style="font-size:1rem;">Scanning images, please wait...</p>
      </div>`;
  ...
  if (document.getElementById('featured-grid')) {
    renderFeatured(); // async, no await — loads progressively
  }
  if (document.querySelector('.category-card')) {
    updateCategoryImages(); // async, no await — updates in background
  }
});
